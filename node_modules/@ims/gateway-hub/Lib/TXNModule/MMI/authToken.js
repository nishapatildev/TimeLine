...
module.exports.frameMessage =  function frameMessage (transXpedia) {
	return new Promise ((resolved, rejected) => {
		try {
			let request = {
                    MapMyIndiaReqDTO : {
                        "requestString": `grant_type=client_credentials&client_id=${transXpedia.clientData.clientId}&client_secret=${transXpedia.clientData.clientSecret}`,
                        "operationName": "OAuth"
                    },
                    sessionContext: {
		...
		try {
            ...
            .then((transXpedia)=>{
					...
					if(transXpedia.xmlJson.Response.status.replyCode == 0){
						...
						transXpedia.responseData.accessToken = responseString.access_token;
					}
					else{
